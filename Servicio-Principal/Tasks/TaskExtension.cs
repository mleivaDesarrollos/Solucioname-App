...
namespace Servicio_Principal
{
    public static class TaskExtension
    {
        ...
        /// <returns></returns>
        public static async Task<TResult> TimeoutAfter<TResult>(this Task<TResult> task, double dblTimeout)
        {
            // Convert from double to Timespan
            TimeSpan timeout = TimeSpan.FromMilliseconds(dblTimeout);
        ...
        /// <param name="timeout"></param>
        /// <returns></returns>
        public static async Task TimeoutAfter(this Task task, double dblTimeout)
        {
            // Convert from double to Timespan
            TimeSpan timeout = TimeSpan.FromMilliseconds(dblTimeout);
            using (var timeoutCancellationTokenSource = new CancellationTokenSource()) {
